    ...
    if (ledTaskQueue != NULL)
    {
          ...
          pdPASS)
      {
...
#ifdef IS_BTCLOCK_S3
          uint endPixel = pixels.numPixels() - 1;
#else
          uint endPixel = 0;
#endif
          pixels.setPixelColor(endPixel, pixels.Color(255, 0, 0));
          pixels.show();
        ...
        case LED_EFFECT_START_TIMER:
        {
          ...
          uint startPixel = 0;
#else
          uint startPixel = (NEOPIXEL_COUNT - 1);
#endif

          pixels.setPixelColor(startPixel, pixels.Color(255, 0, 0));
          pixels.show();
